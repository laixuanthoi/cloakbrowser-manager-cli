...
def test_cli_profile_outputs_redact_secrets(runner):
    create_result = runner.invoke(cli, [
        "--json", "profile", "create", "redact-cli",
        "--proxy", "http://user:pass@example.com:8080",
        "--license-key", "cb_1234567890abcdef",
    ])
    assert create_result.exit_code == 0, create_result.output
    created = json.loads(create_result.output)
    assert "pass" not in create_result.output
    assert "cb_1234567890abcdef" not in create_result.output
    assert created["proxy"] == "http://user:****@example.com:8080"
    assert created["license_key"] == "cb_1…cdef"

    show_result = runner.invoke(cli, ["--json", "profile", "show", "redact-cli"])
    assert show_result.exit_code == 0, show_result.output
    assert "pass" not in show_result.output
    assert "cb_1234567890abcdef" not in show_result.output

    list_result = runner.invoke(cli, ["--json", "list"])
    assert list_result.exit_code == 0, list_result.output
    assert "pass" not in list_result.output
    assert "cb_1234567890abcdef" not in list_result.output


def test_config_get_license_key_redacts(runner):
    runner.invoke(cli, ["config", "set", "--license-key", "cb_1234567890abcdef"])

    result = runner.invoke(cli, ["config", "get", "license_key"])

    assert result.exit_code == 0
    assert "cb_1234567890abcdef" not in result.output
    assert "****" in result.output


def test_launch_rejects_non_http_url(runner):
    runner.invoke(cli, ["profile", "create", "launch-url-guard"])

    result = runner.invoke(cli, ["launch", "launch-url-guard", "--url", "file:///etc/passwd"])

    assert "Launch URL must be an absolute http(s) URL" in result.output


def test_serve_refuses_non_loopback_without_auth(runner):
...
def test_safe_delete_rejects_profiles_root(temp_data_dir):
    from cloakbrowser_manager_cli.core import utils

    assert utils.is_managed_profile_data_dir(temp_data_dir / "profiles") is False
    assert utils.delete_profile_data_dir(temp_data_dir / "profiles", ignore_errors=True) is False
